(function ($, window, document) {
  ...
  document.querySelectorAll(".menu-left li").forEach((item) => {
    item.addEventListener("mouseenter", () => {
      ...
      document
        .querySelectorAll(`.menu-content[data-content="${target}"]`)
  ...
  var tp_obj = {
    ...
    owl_slider: function () {
      ...
      $slider.on("prepare.owl.carousel", function (event) {
        event.data = $("<" + event.relatedTarget.settings.itemElement + "/>")
          .addClass(event.relatedTarget.options.itemClass)
          .attr("data-index", event.item.count)
      ...
      $slider.on("translated.owl.carousel", function (event) {
        ...
        $.each(
          $(
            ".owl-item.active .tp_animate_when_visible",
            $(event.currentTarget)
          ),
          function (index, val) {
        ...
        $.each(
          $(
            ".owl-item:not(.active) .tp_animate_when_visible",
            $(event.currentTarget)
          ),
          function (index, val) {
      ...
      $slider.on("changed.owl.carousel", function (event) {
        ...
        if (isNaN(itendIndex)) itendIndex = 0;
    ...
    phone_menu: function () {
      ...
      $(".tp-main-menu ").on("click", ".phone_menu", function (e) {
        ...
        if (!item.hasClass("ltm_icon_active")) {
          item
            .closest(".tp-main-menu")
            .find(".tp-menu")
            .removeClass("tp-mainmenu-mobile");
        } else {
          item
            .closest(".tp-main-menu")
            .find(".tp-menu")
            .addClass("tp-mainmenu-mobile");
        }
      });
      ...
      $(document).on("click", "body", function (e) {
        ...
        if (
          $(e.target).parents(".tp-menu").length === 1 ||
          $(e.target).parents(".tagpoint-wrap-logo").length === 1 ||
          $(e.target).parents(".tp-main-menu").length === 1
        ) {
        } else {
                ...
                if (value === 0) {
                  ...
                  circle.setText(
                    '<span style="font-size:' +
                      ...
                      'px;">' +
                      value +
                      '<i>%</i></span><h5 class="progress_bar_title" style="font-size:' +
                      labelfsize +
                      "px;color:" +
                      percentfontcolor +
                      ';">' +
      ...
      $("body").on("submit", "#tp-form", function (event) {
        ...
        form
          ...
          .each(function () {
            ...
            if (
              ...
              !pattren.test(jQuery.trim(jQuery(this).val()))
            ) {
              jQuery(this).css("border-color", "red"); //change border color to red
              validated = false; //set do not proceed flag
            }
        ...
        if (validated) {
          tp_obj.tp_ajax(data).done(function (text) {
            form
              .find(".tpsend-btn")
              .after("<p class='msg_feadback'> " + text + "</p> ");
          });
        }
      });
    })(jQuery);
  ...
  $(window).on("load", function () {
    ...
    if ($m_slider.length) {
      $(".owl-item.active .tp_animate_when_visible").each(function (
        index,
        val
      ) {
